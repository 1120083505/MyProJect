...
namespace mvcProject.Areas.SP.Controllers
{
    public class WarehouseReceiptController : Controller
    {
        ...
        public JsonResult GetList(GridPager pager, string queryStr)
        {
            ...
            GetQuery(qList);
            int total = 0;
            query.SqlIn.Where(VSP_SaleMain.Columns.Ext1).IsEqualTo("0");
            query.SqlNotIn.Where(VSP_SaleMain.Columns.Ext1).IsEqualTo("0");
            query.SqlTotal.Where(VSP_SaleMain.Columns.Ext1).IsEqualTo("0");
        ...
        public void GetQuery(List<QueryModel> qList, string[] coulums)
        {
            ...
            {
                //精确查询  
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Id);
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.FormId);
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Warehouse);
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Customer);
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.CustomerId);
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.AdminId);
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Ext1);
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Ext2);
                query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Ext3);

            }
            else
            {
                //模糊查询  
                query.Like(qList, VSP_ShangPinInfo.Columns.Id);
                query.Like(qList, VSP_ShangPinInfo.Columns.FormId);
                query.Like(qList, VSP_ShangPinInfo.Columns.Warehouse);
                query.Like(qList, VSP_ShangPinInfo.Columns.Customer);
                query.Like(qList, VSP_ShangPinInfo.Columns.CustomerId);
                query.Like(qList, VSP_ShangPinInfo.Columns.AdminId);
                query.Like(qList, VSP_ShangPinInfo.Columns.Ext1);
                query.Like(qList, VSP_ShangPinInfo.Columns.Ext2);
                query.Like(qList, VSP_ShangPinInfo.Columns.Ext3);
            }
            query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Iswarehouse);
            query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Num);
            query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.Price);
            query.IsEqualTo(qList, VSP_ShangPinInfo.Columns.SumPrice);
            query.IsGreaterThanOrEqualTo(qList, VSP_ShangPinInfo.Columns.PubDate);
            query.IsLessThanOrEqualTo(qList, VSP_ShangPinInfo.Columns.PubDate);
        ...
        [HttpPost]
        [PowerFilter]
        public JsonResult Create(SP_ShangPinInfo model)
        {
            try
            {
            ...
            GUI.InsertLog("商品明细", "新增", msg + ":" + model.Id + ",商品名称:" + model.FormId + ",是否进出库:" + model.Iswarehouse + ",数量:" + model.Num + ",价格:" + model.Price + ",总价:" + model.SumPrice + ",所属仓库:" + model.Warehouse + ",客户:" + model.Customer + ",客户ID:" + model.CustomerId + ",录入人:" + model.AdminId + ",录入时间:" + model.PubDate + ",:" + model.Ext1 + ",:" + model.Ext2 + ",:" + model.Ext3 + "");
            return ControllerHelper.jsonresult(status, msg);
        }
        #endregion
        #region Edit  
        ...
        [HttpPost]
        [PowerFilter]
        public JsonResult Edit(SP_ShangPinInfo model)
        {
            try
            {
                SP_ShangPinInfo entity = new SP_ShangPinInfo(model.Id);
                if (!string.IsNullOrEmpty(entity.Id))
                {
                    LogStr = ""
                 + "Id:" + model.Id
                 + ",商品名称:" + model.FormId
                 + ",是否进出库:" + model.Iswarehouse
                 + ",数量:" + model.Num
                 + ",价格:" + model.Price
                 + ",总价:" + model.SumPrice
                 ...
                 + ",:" + model.Ext3
                    ...
                    entity.Iswarehouse = model.Iswarehouse;
                    entity.Num = model.Num;
                    entity.Price = model.Price;
                    entity.SumPrice = model.SumPrice;
            ...
            GUI.InsertLog("商品明细", "编辑", msg + LogStr);
            return ControllerHelper.jsonresult(status, msg);
        }
        #endregion
        #region Delete  
        ...
        public JsonResult Delete(string id)
        {
            if (!string.IsNullOrWhiteSpace(id))
            {
                ...
                {
                    //这里书写需要删除的逻辑代码 
                    new Delete().From<SP_ShangPinInfo>().Where(SP_ShangPinInfo.Columns.Id).In(id.Split(','))
                    .Execute();
                    status = 1;
            ...
            GUI.InsertLog("商品明细", "删除", msg + LogStr);
            return ControllerHelper.jsonresult(status, msg);
        }
        #endregion
        ...
        public ActionResult Export(string type, string queryStr)
        {
            ...
            List<QueryModel> qList = c.getQList(queryStr);
            query.pager = pager;
            query.TableName = SP_ShangPinInfo.Schema;
            query.init();
            GetQuery(qList);
            List<SP_ShangPinInfo> list = query.Paged().ExecuteTypedList<SP_ShangPinInfo>();
            LocalReport localReport = new LocalReport();
            try
            {
                localReport.ReportPath = Server.MapPath("~/Report/SP_ShangPinInfo.rdlc");
                ...
                string ExportName = "商品明细";
                GUI.InsertLog("商品明细", "导出", "导出[" + ExportName + "]成功！[" + type.ToLower() + "]");
                switch (type.ToLower())
                {
                    case "pdf":
                        ExportName += ".pdf";
